...
def midinote_to_name(midinote):
    """
    将MIDI音高数字转换为音符名称（例如：60 -> C4）。
    C4 (中央C) 对应 MIDI 音高 60。
    """
    note_names = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]
    
    # 确保音高在有效范围内
    if not (0 <= midinote <= 127):
        return "N/A" # 或者抛出错误，根据需要

    # 计算音符名称的索引
    note_index = midinote % 12
    note_name = note_names[note_index]

    # 计算八度数 (MIDI C0 是音高12，通常我们从 C-1 或 C0 开始八度计数)
    # 约定：MIDI音高60是C4
    octave = (midinote // 12) - 1 # MIDI C0 是八度 -1 或 0，取决于习惯。-1 使得 C4=60
...
def midi_file_to_sequence(midi_filepath, track_index=0, quantization_level=0.25):
    ...
    try:
        ...
        # 获取 MIDI 文件的 ticks_per_beat
        ticks_per_beat = mid.ticks_per_beat

        # 寻找初始 tempo（如果存在的话）
        current_tempo = mido.bpm2tempo(120) # 默认 120 BPM
        for msg in mid.tracks[0]:
            if msg.type == 'set_tempo':
                current_tempo = msg.tempo
                break

        active_note_start_beats = {} # {midi_note: start_beats}
        note_events_in_beats = [] # 存储 (start_beats, end_beats, note)
        ...
        for start_beats, _, note in note_events_in_beats:
            ...
            if quantized_start_beats > last_quantized_time_processed:
                ...
                notes_for_current_quantized_time.clear() # 清空集合，准备收集新拍的音符
        ...
        print(f"成功从 '{midi_filepath}' 导入 {len(sequence)} 个元素。")
        return sequence
        ...
        print(f"MIDI 解析错误细节: {e}") # 打印详细错误信息
        return []
    ...
    def update_melody_listbox(self):
        ...
        for i, item in enumerate(custom_melody_sequence):
            ...
            if isinstance(item, int):
                # --- 修复：使用我们自己实现的 midinote_to_name ---
                display_text = f"音符: {item} ({midinote_to_name(item)})"
                # --- 修复结束 ---
